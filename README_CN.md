...
**本地优先的编码 Agent 运行时。Tauri 桌面工作台，底层是本地 `codepapr-server` 宿主。**

CodePapr 是一个围绕 LLM 前缀缓存效率设计的本地编码 Agent 系统。主 Agent 把任务分派给 **Explore**（代码分析）、**Scout**（网页搜索）、**Mentor**（架构指导）三个内置子代理，另有 **Verifier**（Goal 验收）、**Compactor**（上下文压缩）、**记忆管家**（维护 `.CodePapr/MEMORY.md`）三个运行时内部代理，也支持自定义子代理。文件读写、命令执行、Git 操作、浏览器预览、LSP 诊断都在本地完成。
...
| **宿主 / 客户端拆分** | 桌面端、CLI 以及未来的客户端都通过 JSON-RPC 连同一个本地 `codepapr-server` |
| **多 Agent 协作** | 主 Agent 通过 `task` 工具把任务分派给 Explore/Scout/Mentor 及自定义子代理 |
| **TodoList 任务规划** | Agent 自动创建并跟踪任务清单，支持进度汇报与重规划 |
| **项目记忆（自动维护）** | 偏好、约束和验证过的事实由记忆管家整理进 `.CodePapr/MEMORY.md`；可读、可改、可 diff，面板就是它的编辑器，保存后下一回合生效。 |
...
| **Git 集成** | 8 个 Git action + diff 面板 + 安全回退（备份引用+撤销） |
...
CodePapr **不是**单体 Tauri 应用。Rust 领域逻辑在共享库 crate 中，由独立的宿主守护进程对外服务，客户端只通过 JSON-RPC 调用宿主。
...
| `.CodePapr/MEMORY.md` | 跨会话项目记忆，由记忆管家维护；面板是它的编辑器，每回合注入会话引导 |
| `.CodePapr/project.sqlite` | 项目级状态：会话、检查点、ProjectGraph 缓存 |
...
| **compactor**（内部） | 上下文压缩——确定性骨架仍超预算时做一次二级摘要 | `compactionModel` 档位（fast/primary） | 无 |
| **memory-curator**（内部） | 维护 `.CodePapr/MEMORY.md`（回合交付 / 压缩前两个卡点） | `compactionModel` 档位 | 无（纯推理） |

主 Agent 通过 `task` 工具分派子代理。每个子代理有**独立的 Session 和空白上下文**，只拿到委派的任务描述，主 Agent 的历史对话不会掺入。子代理整体超时 5 分钟，单次工具调用 90 秒。

> **Verifier**、**Compactor** 与 **memory-curator** 是运行时控制的内部代理（`internal: true`）——不经 `task` 工具暴露。Verifier 由 GoalRunner 验收循环调用；Compactor 由上下文压缩管线（轮间 + mid-loop）在确定性骨架仍超预算时做一次二级摘要；memory-curator 由记忆管线在交付 / 压缩前两个卡点调用。Compactor 与 memory-curator 零工具纯推理，墙钟预算沿用子代理默认 20 分钟。
