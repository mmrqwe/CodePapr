...
const MODE_INTROS: Record<PromptLang, Record<PromptMode, string[]>> = {
  'zh-CN': {
    ...
    app: [
      ...
      '声明 inbox 的插件默认停靠右侧栏：surface 用 {type:"panel", width, height}，不要写 overlay+position；只有无 inbox 的自刷新小部件才用 overlay 浮窗。',
      ...
      '      surface: { type: "panel", width: 420, height: 280 },',
  ...
  'zh-TW': {
    ...
    app: [
      ...
      '宣告 inbox 的外掛預設停靠右側欄：surface 用 {type:"panel", width, height}，不要寫 overlay+position；只有無 inbox 的自刷新小元件才用 overlay 浮窗。',
  ...
  en: {
    ...
    app: [
      ...
      'Plugins that declare inbox default to the right dock: use surface {type:"panel", width, height} and do not write overlay+position; only self-refreshing widgets without inbox use the overlay float.',
...
const COMMON_CONSTRAINTS: Record<PromptLang, string[]> = {
  'zh-CN': [
    ...
    '- 高危命令（如 git push --force / git reset --hard / find -delete / 管道给 sh / rm 目标含变量）需用户一次性确认，批准时会自动创建检查点。被确认框拦下或被用户拒绝时：绝不改写、拆分、加引号或用变量替换来绕过检测——这会剥夺用户的否决权；正确做法是用 question 工具说明理由请求授权，或改用非破坏性等效命令（--force-with-lease、内置检查点回滚、先 read 确认删除范围）。',
