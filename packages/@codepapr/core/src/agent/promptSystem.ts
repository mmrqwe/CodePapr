...
const MODE_INTROS: Record<PromptLang, Record<PromptMode, string[]>> = {
  'zh-CN': {
    ...
    app: [
      ...
      '| tools | 工具白名单（15个可选）| ["read", "grep", "web_search"] |',
      '| maxToolRounds | 最大工具轮数（默认20，上限50）| 15 |',
      '',
      'Agent 工具白名单：read（读文件）, grep（内容搜索）, list（目录浏览）, graph（代码符号图）',
      'web_search（网络搜索）, web_fetch（抓取网页）, write（写文件）, edit（编辑）',
      'exec（执行命令）, lsp（语言服务器）, diagnostics（诊断）, read_image（读图片）',
      '拥有 workspace:write/exec 权限才能使用 write/edit/exec 工具。',
  ...
  'zh-TW': {
    ...
    app: [
      ...
      '| tools | 工具白名單 | ["read", "grep", "web_search"] |',
      '| maxToolRounds | 最大工具輪數（預設20，上限50）| 15 |',
      '',
      'Agent 工具白名單：read, grep, list, graph, web_search, web_fetch,',
      'write, edit, exec, lsp, diagnostics, read_image, web_download',
      '擁有 workspace:write/exec 權限才能使用 write/edit/exec 工具。',
  ...
  en: {
    ...
    app: [
      ...
      '| tools | Tool whitelist (15 available) | ["read", "web_search"] |',
      '| maxToolRounds | Max tool rounds (default 20, max 50) | 15 |',
      '',
      'Agent tool whitelist: read, grep, list, graph, web_search, web_fetch,',
      'write, edit, exec, lsp, diagnostics, read_image, web_download.',
      'Write/edit/exec tools require workspace:write/exec permissions.',
