...
const MODE_INTROS: Record<PromptLang, Record<PromptMode, string[]>> = {
  'zh-CN': {
    ...
    app: [
      ...
      '③ 直接调用 app_render：传入 appId、title、html，以及可选的 permissions、agents、level、icon；后端 app 还需 files（如 server.js）+ command/args/port。app_render 会自动写入 manifest.json 和 index.html——不要先手动写文件',
      ...
      'L3 系统：+ Agent 文件写入（write/edit/patch 限 app sandbox 目录）+ 终端执行（bash 可及整个工作区）。需用户在设置中全局开启。',
      '',
      ...
      'server.js 优先只用 Node 内置模块（http/fs/path 等），除非项目已安装所需依赖。最小模板：',
      '```',
      ...
      '}).listen(<port>);  // port 与 app_render 的 port 参数一致',
      '```',
      ...
      '- 应用内数据持久化默认用 papr.db（set/get/delete/keys）并声明 storage:read/write 权限——不要用内存变量或 localStorage',
  ...
  'zh-TW': {
    ...
    app: [
      ...
      '③ 直接調用 app_render：傳入 appId、title、html，以及可選的 permissions、agents、level、icon；後端 app 還需 files（如 server.js）+ command/args/port。app_render 會自動寫入 manifest.json 和 index.html——不要先手動寫檔案',
      ...
      'L3 系統：+ Agent 檔案寫入（write/edit/patch 限 app sandbox 目錄）+ 終端執行（bash 可及整個工作區）。需使用者在設定中全域開啟。',
      '',
      ...
      'server.js 優先只用 Node 內建模組（http/fs/path 等），除非專案已安裝所需依賴。最小範本：',
      '```',
      'const http = require("http");',
      'http.createServer((req, res) => {',
      '  res.setHeader("Access-Control-Allow-Origin", "*");',
      ...
      '}).listen(<port>);  // port 與 app_render 的 port 參數一致',
      '```',
      ...
      '- 應用內資料持久化預設用 papr.db（set/get/delete/keys）並宣告 storage:read/write 權限——不要用記憶體變數或 localStorage',
  ...
  en: {
    ...
    app: [
      ...
      '③ Call app_render directly: pass appId, title, html, and optional permissions, agents, level, icon; backend apps also need files (e.g. server.js) + command/args/port. app_render writes manifest.json and index.html itself — do NOT write the files manually first',
      ...
      'L3 System: + Agent file writes (write/edit/patch confined to the app sandbox dir) + shell execution (bash can reach the whole workspace). Requires user to enable L3 globally in settings.',
      '',
      ...
      'Prefer Node built-in modules (http/fs/path) in server.js unless the project already has the needed dependencies. Minimal template:',
      '```',
      'const http = require("http");',
      'http.createServer((req, res) => {',
      '  res.setHeader("Access-Control-Allow-Origin", "*");',
      ...
      '}).listen(<port>);  // port must match the app_render port parameter',
      '```',
      ...
      '- Use papr.db (set/get/delete/keys) as the default in-app persistence and declare storage:read/write — not in-memory variables or localStorage',
