...
const MODE_INTROS: Record<PromptLang, Record<PromptMode, string[]>> = {
  'zh-CN': {
    ...
    app: [
      ...
      'L1 Runtime（默认）：papr.db 存储 + papr.fs 文件 + AI Agent（只读工具）。适合 Todo、笔记。',
      'L2 联网：+ papr.http + Agent 联网搜索 + MCP 工具。适合数据看板、API 调用。',
      'L3 系统：+ 文件写入 + 终端 + Git。需用户在设置中全局开启。适合代码重构工具。',
      '',
      ...
      '| workspace:read | Agent 只读工具 | L1 |',
      '| http:get, http:post | papr.http | L2 |',
      '| workspace:write | Agent 写入工具 | L3 |',
      '| workspace:exec | Agent 执行工具 | L3 |',

      '## Agent 定义',
      '在 agents 参数中声明 app 可调用的 AI Agent。每个 Agent 是一个可运行多轮工具调用的子代理。',
      '| 字段 | 说明 | 示例 |',
      '| name | Agent 名称 | "assistant", "analyst" |',
      '| model | main（用户主模型）/ fast / mentor / 具体模型名 | "main" |',
      '| systemPrompt | 自定义系统提示词 | "你是数据分析专家" |',
      '| tools | 工具白名单（可选，不声明=继承主 Agent 全部工具）| ["read", "web_search"] |',
      '| maxToolRounds | 最大工具轮数（默认20，上限50）| 15 |',
      '',
      '不声明 tools → Agent 拥有主 Agent 的全部工具（包括 MCP 工具、skill_load 等）。',
      '声明 tools → 仅使用白名单中的工具。始终排除：task（委派子代理）、app_render（套娃生成）。',
  ...
  'zh-TW': {
    ...
    app: [
      ...
      '## 權限模型',
      '| 權限 | 解鎖功能 |',
      '| storage:read, storage:write | papr.db |',
      '| http:get, http:post | papr.http |',
      '| fs:read, fs:write | papr.fs |',
      '| agent:run:<name> | papr.agent.run({agent: name}) |',
      '| workspace:read | Agent 工具：read, grep, list, graph, lsp |',
      '| workspace:write | Agent 工具：write, edit, patch |',
      '| workspace:exec | Agent 工具：exec, shell |',

      '## Agent 定義',
      '在 agents 參數中宣告 app 可調用的 AI Agent。每個 Agent 是一個可運行多輪工具調用的子代理。',
      '| name | Agent 名稱 | "assistant" |',
      '| model | main / fast / mentor / 具體模型名 | "main" |',
      '| systemPrompt | 自定義系統提示詞 | "你是資料分析專家" |',
      '| tools | 工具白名單（可選，不宣告=繼承主 Agent 全部工具）| ["read", "web_search"] |',
      '| maxToolRounds | 最大工具輪數（預設20，上限50）| 15 |',
      '',
      '不宣告 tools → Agent 擁有主 Agent 的全部工具（包括 MCP 工具、skill_load 等）。',
      '宣告 tools → 僅使用白名單中的工具。始終排除：task、app_render。',
      '高危工具（write/edit/exec）仍需要 manifest.permissions 中宣告 workspace:write/exec 權限。',
  ...
  en: {
    ...
    app: [
      ...
      '| fs:read, fs:write | papr.fs |',
      '| agent:run:<name> | papr.agent.run({agent: name}) |',
      '| workspace:read | Agent tools: read, grep, list, graph, lsp |',
      '| workspace:write | Agent tools: write, edit, patch |',
      '| workspace:exec | Agent tools: exec, shell |',
      ...
      '| model | main (user primary) / fast / mentor / model ID | "main" |',
      '| systemPrompt | Custom system prompt | "You are a data analyst" |',
      '| tools | Tool whitelist (optional, omit = inherit all main agent tools) | ["read", "web_search"] |',
      '| maxToolRounds | Max tool rounds (default 20, max 50) | 15 |',
      '',
      'Omit tools → agent inherits ALL main agent tools (including MCP tools, skill_load, etc.).',
      'Specify tools → only those tools are available. Always excluded: task (sub-delegation), app_render (nesting).',
