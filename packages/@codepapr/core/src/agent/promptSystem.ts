...
const MODE_INTROS: Record<PromptLang, Record<PromptMode, string[]>> = {
  'zh-CN': {
    ...
    app: [
      ...
      '## .papr 应用是什么',
      '每个 app = manifest.json（元数据 + 权限 + Agent 定义） + index.html（入口 HTML）。',
      ...
      '## 前端规范',
      '- 完整 HTML 文档作为入口；复杂样式/脚本用 files 写 app.css / app.js，再用相对路径引用（不要上构建链）',
      '- 图表库：network:true 才可通过 CDN <script src="..."> 引用（D3、ECharts、Mermaid、MapLibre、Leaflet、Three.js）；network:false 必须内联或放进 files',
      ...
      '## 关键约束',
      '- 已有应用且用户未要求重做时，只改用户指出的问题（布局、筛选、文案、交互），禁止再走一遍全量生成；用 patch/edit 或对同一 appId 做增量更新',
  ...
  'zh-TW': {
    ...
    app: [
      ...
      '## .papr 應用是什麼',
      '每個 app = manifest.json（元資料 + 權限 + Agent 定義） + index.html（入口 HTML）。',
      ...
      '## 前端規範',
      '- 完整 HTML 文件作為入口；複雜樣式/腳本用 files 寫 app.css / app.js，再用相對路徑引用（不要上構建鏈）',
      '- 圖表庫：network:true 才可通過 CDN 引用（D3、ECharts、Mermaid、MapLibre、Leaflet、Three.js）；network:false 必須內聯或放進 files',
      ...
      '## 關鍵約束',
      '- 已有應用且用戶未要求重做時，只改用戶指出的問題（佈局、篩選、文案、互動），禁止再走一遍全量生成；用 patch/edit 或對同一 appId 做增量更新',
  ...
  en: {
    ...
    app: [
      ...
      '## What is a .papr App',
      'Each app = manifest.json (metadata + permissions + Agent definitions) + index.html (entry HTML).',
      ...
      '- Complete HTML document as the entry; put extra CSS/JS in files as app.css / app.js and link them with relative paths (no build chain)',
      '- Chart libraries via CDN only when network:true (D3, ECharts, Mermaid, MapLibre, Leaflet, Three.js); when network:false, inline or put them in files',
      ...
      '    appId: "todo-app",',
      '    title: "AI Todo App",',
      '    local: "none",   // no project file access needed',
      ...
      '## Key Rules',
      '- If an app already exists and the user did not ask to rebuild it, only change what they pointed out (layout, filters, copy, interaction). Do not regenerate from scratch; patch/edit or incrementally update the same appId',
