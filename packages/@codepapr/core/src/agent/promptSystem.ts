...
const MODE_INTROS: Record<PromptLang, Record<PromptMode, string[]>> = {
  'zh-CN': {
    ...
    app: [
      ...
      '③ 用 write（或 edit/patch）把应用写到 `.CodePapr/apps/<appId>/`：manifest.json + 骨架 index.html + css/theme.css + 多个 js/*.js（按职责拆，禁止单文件巨石）。不要把 html/title/files 传给 app_render',
      ...
      '规则：index.html 禁止大段 style/script；单文件大约超过 200 行就再拆；后续修改只 patch 对应小文件，禁止重新合并。插件同样拆 css + js，不是把逻辑内联回 HTML。',
      '',
      ...
      '→ 用 write 写入 `.CodePapr/apps/stock-ticker/index.html` 骨架 + css/theme.css + js/api.js + js/main.js（papr.http.get + papr.db）',
  ...
  'zh-TW': {
    ...
    app: [
      ...
      '③ 用 write（或 edit/patch）把應用寫到 `.CodePapr/apps/<appId>/`：manifest.json + 骨架 index.html + css/theme.css + 多個 js/*.js（按職責拆，禁止單檔巨石）。不要把 html/title/files 傳給 app_render',
      ...
      '規則：index.html 禁止大段 style/script；單檔大約超過 200 行就再拆；後續修改只 patch 對應小檔案，禁止重新合併。外掛同樣拆 css + js，不是把邏輯內聯回 HTML。',
      '',
      '## Papr SDK — 前端可用的全部能力（window.papr）',
      '',
      ...
      '    { spec:"papr/0.1", name: "股票看板", kind: "plugin", local: "none", network: true,',
      '      surface: { type: "overlay", width: 320, height: 200, position: "top-right" } }',
      '→ 用 write 寫入 `.CodePapr/apps/stock-ticker/index.html` 骨架 + css/theme.css + js/api.js + js/main.js（papr.http.get + papr.db）',
  ...
  en: {
    ...
    app: [
      ...
      '③ Use write (or edit/patch) to put the app in `.CodePapr/apps/<appId>/`: manifest.json + a shell index.html + css/theme.css + several js/*.js files (split by responsibility; no single-file monolith). Do NOT pass html/title/files to app_render',
      ...
      'Rules: no large style/script blocks in index.html; split a file again around 200 lines; later edits patch the small file, never re-merge. Plugins still split css + js — do not inline logic back into HTML.',
      '',
      ...
      '      surface: { type: "overlay", width: 320, height: 200, position: "top-right" } }',
      '→ Write `.CodePapr/apps/stock-ticker/index.html` shell + css/theme.css + js/api.js + js/main.js (papr.http.get + papr.db)',
