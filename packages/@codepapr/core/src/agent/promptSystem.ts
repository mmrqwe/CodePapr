...
const MODE_INTROS: Record<PromptLang, Record<PromptMode, string[]>> = {
  'zh-CN': {
    ...
    app: [
      ...
      '插件 HTML 按小窗写：信息密度高，不要自做顶栏（宿主提供拖动和关闭）。默认三文件：index.html + css/theme.css + js/main.js。',

      '## 工作流程',
      '① app_list 检查现有应用（避免覆盖同名 app）',
      '② 探索数据：用 list（目录树）/ read（读文件）/ grep（搜内容）/ glob（按名查找）了解数据源结构',
      '③ 用 write（或 edit/patch）把应用写到 `.CodePapr/apps/<appId>/`：manifest.json + 骨架 index.html + css/theme.css + 多个 js/*.js（按职责拆，禁止单文件巨石；插件默认三文件）。不要把 html/title/files 传给 app_render',
      ...
      '规则：index.html 禁止大段 style/script；单文件大约超过 200 行就再拆；后续修改只 patch 对应小文件，禁止重新合并。插件默认只要 index.html + css/theme.css + js/main.js，不要一上来拆 5 个文件；涨过约 200 行再拆。禁止把逻辑内联回 HTML。',
      '',
      ...
      '后端服务：需要读写项目数据库、运行复杂查询时，用 write 写 server.js，并在 manifest 中提供 command/args/port。后端进程的工作目录是应用目录 .CodePapr/apps/<appId>/，相对路径只写文件名（如 server.js）。',
      ...
      '⚠️ 前端运行在 codepapr-app:// 沙箱 origin，fetch 后端属于跨域——server.js 必须设置 CORS 响应头。',
      'server.js 必须读 process.env.PORT / process.env.HOST（启动器在端口占用时会改绑并注入这两个变量）。最小模板：',
      '```',
      ...
      '  res.end(JSON.stringify({ ok: true }));  // 在此实现路由',
      '}).listen(port, host);',
      '```',
      ...
      '- 生成后自检：① local/network 与代码实际能力匹配（fetch 外部/papr.http → network:true；agent 写项目 → local:write）② CDN 库 URL 正确可达 ③ 刷新后状态不丢失（用 papr.db）④ 移动端布局不破 ⑤ 没有单文件巨石（index.html 只是骨架，js 已按职责拆开）',
      ...
      '→ write index.html（骨架：link css/theme.css + script type="module" src="js/main.js"）',
      '→ write css/theme.css、js/db.js（papr.db）、js/agent.js（papr.agent.run）、js/ui.js、js/main.js',
  ...
  'zh-TW': {
    ...
    app: [
      ...
      '外掛 HTML 按小窗寫：資訊密度高，不要自做頂欄（宿主提供拖動和關閉）。默認三檔：index.html + css/theme.css + js/main.js。',

      '## 工作流程',
      '① app_list 檢查現有應用（避免覆蓋同名 app）',
      '② 探索資料：用 list（目錄樹）/ read（讀檔案）/ grep（搜內容）/ glob（按名查找）了解資料源結構',
      '③ 用 write（或 edit/patch）把應用寫到 `.CodePapr/apps/<appId>/`：manifest.json + 骨架 index.html + css/theme.css + 多個 js/*.js（按職責拆，禁止單檔巨石；外掛默認三檔）。不要把 html/title/files 傳給 app_render',
      ...
      '規則：index.html 禁止大段 style/script；單檔大約超過 200 行就再拆；後續修改只 patch 對應小檔案，禁止重新合併。外掛默認只要 index.html + css/theme.css + js/main.js，不要一上來拆 5 個檔；漲過約 200 行再拆。禁止把邏輯內聯回 HTML。',
      '',
      '## Papr SDK — 前端可用的全部能力（window.papr）',
      '',
      ...
      '後端服務：需要讀寫專案資料庫、執行複雜查詢時，用 write 寫 server.js，並在 manifest 中提供 command/args/port。後端進程的工作目錄是應用目錄 .CodePapr/apps/<appId>/，相對路徑只寫檔名（如 server.js）。',
      '⚠️ 後端 app 的 HTML 中，API 調用必須用 window.__PAPR_BACKEND_URL 作為 base URL：',
      '  const API = window.__PAPR_BACKEND_URL || "";',
      '  fetch(API + "/api/data")  // -> http://localhost:<port>/api/data',
      '⚠️ 前端執行在 codepapr-app:// 沙箱 origin，fetch 後端屬於跨域——server.js 必須設定 CORS 回應標頭。',
      'server.js 必須讀 process.env.PORT / process.env.HOST（啟動器在埠被佔用時會改綁並注入這兩個變數）。最小範本：',
      '```',
      'const http = require("http");',
      'const port = Number(process.env.PORT) || <port>;',
      'const host = process.env.HOST || "127.0.0.1";',
      ...
      '  if (req.method === "OPTIONS") { res.writeHead(204); return res.end(); }',
      '  res.setHeader("Content-Type", "application/json");',
      '  res.end(JSON.stringify({ ok: true }));  // 在此實作路由',
      '}).listen(port, host);',
      '```',
      ...
      '- 生成後自檢：① local/network 與程式碼實際能力匹配（fetch 外部/papr.http → network:true；agent 寫專案 → local:write）② CDN 庫 URL 正確可達 ③ 重新整理後狀態不丟失（用 papr.db）④ 行動版版面不破 ⑤ 沒有單檔巨石（index.html 只是骨架，js 已按職責拆開）',
  ...
  en: {
    ...
    app: [
      ...
      'Write plugin HTML for a small card: high density, no custom title bar (the host provides drag and close). Default three files: index.html + css/theme.css + js/main.js.',
      ...
      '③ Use write (or edit/patch) to put the app in `.CodePapr/apps/<appId>/`: manifest.json + a shell index.html + css/theme.css + several js/*.js files (split by responsibility; no single-file monolith; plugins default to three files). Do NOT pass html/title/files to app_render',
      ...
      'Rules: no large style/script blocks in index.html; split a file again around 200 lines; later edits patch the small file, never re-merge. Plugins default to index.html + css/theme.css + js/main.js — do not start with five files; split further only past ~200 lines. Do not inline logic back into HTML.',
      '',
      ...
      'Backend: write server.js and put command/args/port in the manifest when you need to read project databases or run complex queries. The backend process cwd is the app directory .CodePapr/apps/<appId>/; relative paths are filenames only (e.g. server.js).',
      '⚠️ In backend apps, API calls MUST use window.__PAPR_BACKEND_URL as base URL:',
      '  const API = window.__PAPR_BACKEND_URL || "";',
      '  fetch(API + "/api/data")  // -> http://localhost:<port>/api/data',
      '⚠️ The frontend runs in a codepapr-app:// sandbox origin, so fetching the backend is cross-origin — server.js MUST set CORS response headers.',
      'server.js MUST read process.env.PORT / process.env.HOST (the launcher reallocates a busy port and injects these). Minimal template:',
      '```',
      'const http = require("http");',
      'const port = Number(process.env.PORT) || <port>;',
      'const host = process.env.HOST || "127.0.0.1";',
      ...
      '  if (req.method === "OPTIONS") { res.writeHead(204); return res.end(); }',
      '  res.setHeader("Content-Type", "application/json");',
      '  res.end(JSON.stringify({ ok: true }));  // implement routes here',
      '}).listen(port, host);',
      '```',
      ...
      '- Self-check before finishing: ① local/network must match the capabilities the code actually uses (external fetch/papr.http → network:true; agent writing to the project → local:write) ② CDN URLs are reachable ③ state survives refresh (papr.db) ④ mobile layout does not break ⑤ no single-file monolith (index.html is a shell; JS is split by responsibility)',
      ...
      '→ write index.html (shell: link css/theme.css + script type="module" src="js/main.js")',
      '→ write css/theme.css, js/db.js (papr.db), js/agent.js (papr.agent.run), js/ui.js, js/main.js',
