  ...
  if (context.runShell) {
    ...
    for (const match of shellMatches) {
      ...
      try {
        ...
        result = result.replace(token, output.trim());
      } catch (error) {
        result = result.replace(token, `[命令执行失败: ${(error as Error).message}]`);
      }
    }
  }
  ...
  if (context.readFile) {
    ...
    for (const match of fileMatches) {
      ...
      try {
        ...
        result = result.replace(token, `${lead}\n\`\`\`\n${content.trim()}\n\`\`\``);
      } catch (error) {
        result = result.replace(token, `${lead}[文件读取失败: ${(error as Error).message}]`);
      }
    }
  }
