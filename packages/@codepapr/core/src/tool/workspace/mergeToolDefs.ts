...
export const NEW_TOOL_DEFINITIONS: IToolDefinition[] = [
  ...
  {
    name: 'question',
    description: '向用户提出明确的问题以收集需求、确认决策或消除歧义。仅在 Plan 模式下使用，且只用于会改变实施方向、无法从上下文推断的关键分歧；不要用作常规审批或逐步骤确认的门禁，用户已明确决策或已下令执行时禁止就同一问题重复提问。可以提供预定义选项供用户选择，用户也可输入自定义想法；如果不提供 options 则为开放式文本回答。用户选中带批准语义的选项（如「批准实施」「开始执行」）即为执行授权，收到后应立即实施，不要再提问或重复输出 Plan。',
    parameters: {
      type: 'object',
      properties: {
