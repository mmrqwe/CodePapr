...
function TreeChevronIcon({ isExpanded }: { isExpanded: boolean }) {
  return (
    <svg
      viewBox="0 0 16 16"
      aria-hidden="true"
      className={`h-3.5 w-3.5 flex-shrink-0 text-fg-muted transition-transform ${
        isExpanded ? 'rotate-90 text-fg-soft' : ''
      }`}
    >
    ...
    <div>
      <button
        ...
        className={`group flex h-6 w-full items-center gap-1.5 rounded-md pr-2 text-left text-[12px] leading-none transition-colors focus:outline-none ${
          isSelected
            ? 'bg-accent-soft text-fg shadow-[inset_2px_0_0_0_rgba(191,219,254,0.95),inset_0_0_0_1px_rgba(96,165,250,0.42)]'
            : isActive
            ? 'bg-base text-fg shadow-[inset_2px_0_0_0_rgba(148,163,184,0.45)]'
            : node.isDir
            ? 'text-fg hover:bg-base'
            : 'text-fg-soft hover:bg-base hover:text-fg'
        } ${isActive ? 'ring-1 ring-info-bg ring-inset' : ''} focus-visible:ring-1 focus-visible:ring-info-bg focus-visible:ring-inset`}
      ...
      >
        ...
        <span className={`min-w-0 truncate ${node.isDir ? 'font-medium text-fg' : ''}`}>
          {node.name}
        </span>
      </button>
...
export function CodingWorkbench({
  ...
  const fileTreeContent = (
    <div className="h-full min-h-0 overflow-y-auto overscroll-contain scrollbar-thin scrollbar-stable px-2 py-2" role="tree" aria-label={t.fileTree}>
      {isLoadingTree && entries.length === 0 && (
        <div className="flex flex-col items-center gap-2 px-2 py-6 text-center text-xs text-fg-muted">
          <span>{t.loadingProject}</span>
          <span className="inline-flex h-1 w-16 overflow-hidden rounded-full bg-raised">
            <span className="status-indicator-bar h-full w-3 rounded-full bg-accent-soft" />
          </span>
        </div>
      )}
      {!isLoadingTree && entries.length === 0 && !folderError && (
        <div className="px-2 py-6 text-center text-xs text-fg-dim">{t.noFiles}</div>
      )}
      {!isLoadingTree && folderError && (
        <div className="flex flex-col items-center gap-2 px-2 py-6 text-center">
          ...
          <button
            ...
            className="rounded-md border border-danger-bg px-2.5 py-1 text-[11px] text-danger transition-colors hover:bg-danger-bg"
          >
            {t.reloadProject}
          </button>
        </div>
      ...
      {!isLoadingTree && !folderError && treeTruncated && (
        <div className="px-2 py-2 text-center text-xs leading-relaxed text-warn">
          {t.fileTreeTruncated}
        </div>
      )}
      {treeNodes.map((node) => (
  ...
  return (
    <div className="flex h-full flex-col select-none">
        ...
        <div className="border-b border-line px-4 py-3">
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <h2 className="text-sm font-semibold text-fg-soft">{t.workbench}</h2>
              <p className="mt-0.5 truncate text-[11px] text-fg-dim">{workspacePath || t.projectEmptyDesc}</p>
            </div>
            <button
              type="button"
              onClick={chooseFolder}
              title={workspacePath ? t.changeFolderTip : t.selectFolderTip}
              className="h-8 flex-shrink-0 rounded-lg border border-accent-soft px-2.5 text-xs font-medium text-accent-text
      ...
      {!workspacePath ? (
        <div className="flex flex-1 flex-col items-center justify-center px-6 text-center text-fg-muted">
          <div className="text-sm font-semibold text-fg-soft">{t.projectEmptyTitle}</div>
            <p className="mt-2 max-w-[260px] text-xs leading-relaxed text-fg-dim">{t.projectEmptyDesc}</p>
          <button
            type="button"
            onClick={chooseFolder}
            className="mt-4 rounded-xl bg-accent px-4 py-2 text-xs font-semibold text-white transition-colors hover:bg-accent"
          >
            {t.selectFolder}
          </button>
          {folderError && <p className="mt-3 text-xs text-danger">{folderError}</p>}
        </div>
      ) : (
        <div className="relative flex min-h-0 flex-1 flex-col">
          {(isWorkspaceSwitching || (isLoadingTree && entries.length === 0)) && (
            <div className="absolute inset-0 z-30 flex items-center justify-center bg-base/80 backdrop-blur-sm">
              <div className="flex flex-col items-center gap-4 rounded-2xl border border-line bg-base px-10 py-8 shadow-2xl">
                <div className="text-sm font-semibold text-fg">{t.workspaceInitLoading}</div>
                <div className="flex flex-col items-center gap-2">
                  ...
                  <div className="h-1.5 w-48 overflow-hidden rounded-full bg-raised">
                    <div className="status-indicator-bar h-full w-10 rounded-full bg-accent-soft" />
                  </div>
                </div>
              </div>
            </div>
          ...
          {!hideProjectSummary && (
            <div className="border-b border-line px-4 py-2.5">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="truncate text-xs font-semibold text-fg">{projectName}</div>
                   ...
                   {settings.debugEnabled && projectGraphLoading && projectGraphProgress && (
                    <span className="flex items-center gap-1.5 text-[10px] text-accent">
                      ...
                      {projectGraphProgress.total > 0 && (
                        <span className="inline-flex h-1.5 w-16 overflow-hidden rounded-full bg-raised">
                          <span
              ...
              {folderError && <div className="mt-2 text-xs text-danger">{folderError}</div>}
              {treeTruncated && <div className="mt-2 text-xs text-warn">{t.fileTreeTruncated}</div>}
            </div>
          )}
          ...
          {previewPlacement === 'hidden' ? (
            <section className="flex h-full min-h-0 flex-col overflow-hidden bg-base">
              <div className="flex items-center justify-between border-b border-line px-4 py-2">
                <div className="flex items-center gap-2">
                  ...
                  >
                    {t.appDockTab}
                    {appCount > 0 && (
                      <span className="inline-flex h-4 min-w-[16px] items-center justify-center rounded-full bg-accent-soft px-1 text-[10px] font-semibold text-fg">
                        {appCount}
                      </span>
                    )}
                  </button>
              ...
              {hiddenSidebarTab === 'tree' && isLoadingTree && entries.length > 0 && (
                <div className="flex items-center gap-2 border-b border-line px-4 py-2 text-[10px] text-fg-muted">
                  <span>{t.loadingProject}</span>
                  <span className="inline-flex h-1 w-10 overflow-hidden rounded-full bg-raised">
                    <span className="status-indicator-bar h-full w-3 rounded-full bg-accent-soft" />
                  </span>
                </div>
              )}
            ...
            <SplitPane
              ...
              first={
                <section className="flex h-full min-h-0 flex-col overflow-hidden border-r border-line bg-base">
