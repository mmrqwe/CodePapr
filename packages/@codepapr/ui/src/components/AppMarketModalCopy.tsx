...
export function copy(lang: Lang | undefined) {
  if (lang === 'en') {
    return {
      ...
      uninstallKeepData: 'Keep app data (papr.db)',
      uninstallKeepDataBody: 'Removes the app files but keeps its local data (db.sqlite) in the folder so a future reinstall can restore it.',
  ...
  if (lang === 'zh-TW') {
    return {
      ...
      uninstallKeepData: '保留本地數據（papr.db）',
      uninstallKeepDataBody: '只刪除應用程式檔，本地數據（db.sqlite）留在原目錄，將來重裝可恢復。',
  ...
  return {
    ...
    uninstallKeepData: '保留本地数据（papr.db）',
    uninstallKeepDataBody: '只删除应用文件，本地数据（db.sqlite）留在原目录，将来重装可恢复。',
