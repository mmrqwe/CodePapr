...
// 不依赖任何 props 的组件提升为模块级常量：流式渲染时每帧重建
// markdownComponents 会让 ReactMarkdown 内部缓存失效，静态部分只建一次。
const STATIC_MARKDOWN_COMPONENTS: Partial<Components> = {
  ...
  // react-markdown 会传入 node prop，直接展开到 DOM 会触发
  // "unknown DOM property" 告警；这里显式剔除。
  input: ({ checked, type, node, ...rest }) => {
    void node;
    if (type === 'checkbox') {
      return (
        <input
...
export const MarkdownRenderer = memo(function MarkdownRenderer({
  ...
  // 依赖 props 的组件用 useMemo 记忆化，只在 copyLabel/回调变化时重建。
