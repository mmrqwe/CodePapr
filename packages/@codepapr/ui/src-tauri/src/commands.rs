...
#[tauri::command]
pub async fn save_memory_candidate(app: AppHandle, workspace_path: String, candidate_json: String) -> Result<Value, String> {
    rpc(&app, "db/saveMemoryCandidate", json!({ "workspacePath": workspace_path, "candidateJson": candidate_json })).await
}
...
pub async fn reject_memory_candidate(
    ...
    rpc_ok(&app, "db/rejectMemoryCandidate", json!({
        "workspacePath": workspace_path, "candidateId": candidate_id, "reason": reason,
    })).await
}

#[tauri::command]
pub async fn forget_memory_entry(
    app: AppHandle,
    workspace_path: String,
    entry_id: String,
    reason: Option<String>,
) -> Result<(), String> {
    rpc_ok(&app, "db/forgetMemoryEntry", json!({
        "workspacePath": workspace_path, "entryId": entry_id, "reason": reason,
    })).await
}
...
#[tauri::command]
pub async fn load_memory_candidates(app: AppHandle, workspace_path: String, status: Option<String>) -> Result<Value, String> {
    rpc(&app, "db/loadMemoryCandidates", json!({ "workspacePath": workspace_path, "status": status })).await
}

#[tauri::command]
pub async fn project_memory_file(app: AppHandle, workspace_path: String, managed_zone_markdown: String) -> Result<(), String> {
    rpc_ok(&app, "db/projectMemoryFile", json!({
        "workspacePath": workspace_path, "managedZoneMarkdown": managed_zone_markdown,
    })).await
}

#[tauri::command]
pub async fn sync_user_zone_to_ledger(app: AppHandle, workspace_path: String) -> Result<(), String> {
    rpc_ok(&app, "db/syncUserZoneToLedger", json!({ "workspacePath": workspace_path })).await
}

#[tauri::command]
pub async fn ingest_legacy_memory_md(app: AppHandle, workspace_path: String) -> Result<Value, String> {
    rpc(&app, "db/ingestLegacyMemoryMd", json!({ "workspacePath": workspace_path })).await
}

#[tauri::command]
pub async fn update_memory_entry_content(
    app: AppHandle,
    workspace_path: String,
    entry_id: String,
    content: String,
) -> Result<(), String> {
    rpc_ok(&app, "db/updateMemoryEntryContent", json!({
        "workspacePath": workspace_path, "entryId": entry_id, "content": content,
    })).await
}

#[tauri::command]
pub async fn save_memory_recall(app: AppHandle, workspace_path: String, recall_json: String) -> Result<(), String> {
    rpc_ok(&app, "db/saveMemoryRecall", json!({ "workspacePath": workspace_path, "recallJson": recall_json })).await
}

#[tauri::command]
pub async fn archive_memory_recall(app: AppHandle, workspace_path: String, recall_id: String) -> Result<(), String> {
    rpc_ok(&app, "db/archiveMemoryRecall", json!({ "workspacePath": workspace_path, "recallId": recall_id })).await
}

#[tauri::command]
pub async fn load_latest_memory_recall(app: AppHandle, workspace_path: String, session_id: String) -> Result<Value, String> {
    rpc(&app, "db/loadLatestMemoryRecall", json!({ "workspacePath": workspace_path, "sessionId": session_id })).await
}

#[tauri::command]
pub async fn search_memory_for_recall(app: AppHandle, workspace_path: String, query_json: String) -> Result<Value, String> {
    rpc(&app, "db/searchMemoryForRecall", json!({ "workspacePath": workspace_path, "queryJson": query_json })).await
}
