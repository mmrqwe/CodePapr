...
pub fn dispatch(
    ...
    match name {
        "lsp" => dispatch_lsp_action(app, workspace_path, args),
        "workspace_symbol_definition" => {
            navigation(app, workspace_path, args, "textDocument/definition")
        }
        "workspace_symbol_references" => references(app, workspace_path, args),
        "workspace_symbol_hover" => hover(app, workspace_path, args),
        "workspace_document_symbol" => document_symbol(app, workspace_path, args),
        "workspace_workspace_symbol" => workspace_symbol(app, workspace_path, args),
        "workspace_implementation" => {
            navigation(app, workspace_path, args, "textDocument/implementation")
        }
        "workspace_prepare_call_hierarchy" => prepare_call_hierarchy(app, workspace_path, args),
        "workspace_incoming_calls" => call_hierarchy_calls(app, workspace_path, args, true),
        "workspace_outgoing_calls" => call_hierarchy_calls(app, workspace_path, args, false),
        "diagnostics" | "workspace_lsp_diagnostics" => diagnostics(app, workspace_path, args),
        "lsp_edit" => dispatch_lsp_edit(app, workspace_path, args),
        "workspace_rename_symbol" => rename(app, workspace_path, args),
        "workspace_apply_code_action" => apply_code_action(app, workspace_path, args),
        ...
        "workspace_fix_diagnostics" => {
            apply_code_action(app, workspace_path, &with_kind(args, "quickfix"))
        }
        "workspace_format_files" => format_files(app, workspace_path, args),
...
fn dispatch_lsp_action(
    ...
    match action.as_str() {
        "goToDefinition" => navigation(app, workspace_path, args, "textDocument/definition"),
        "findReferences" => references(app, workspace_path, args),
        "hover" => hover(app, workspace_path, args),
        "documentSymbol" => document_symbol(app, workspace_path, args),
        "workspaceSymbol" => workspace_symbol(app, workspace_path, args),
        "goToImplementation" => {
            navigation(app, workspace_path, args, "textDocument/implementation")
        }
        "prepareCallHierarchy" => prepare_call_hierarchy(app, workspace_path, args),
        "incomingCalls" => call_hierarchy_calls(app, workspace_path, args, true),
        "outgoingCalls" => call_hierarchy_calls(app, workspace_path, args, false),
...
fn dispatch_lsp_edit(
    ...
    let action = arg_string(args, "action").ok_or_else(|| "action 必须是字符串".to_string())?;
    match action.as_str() {
        "rename" => rename(app, workspace_path, args),
        "code_action" => apply_code_action(app, workspace_path, args),
        "format" => format_files(app, workspace_path, args),
...
fn navigation(
    ...
    let result = lsp_position_request(app, workspace_path, args, method, json!({}))?;
...
fn hover(
    ...
    let result = lsp_position_request(app, workspace_path, args, "textDocument/hover", json!({}))?;
...
fn call_hierarchy_calls(
    ...
    let result = lsp_request_opened(app, workspace_path, args, method, json!({ "item": item }))?;
...
fn diagnostics(
    ...
    if let Some(path) = relative_path {
        ...
        let _ = open_document(app, workspace_path, &language_id, &path);
        let response =
...
fn lsp_position_request(
    ...
    lsp_request_opened(app, workspace_path, args, method, params)
}
...
fn lsp_request_opened(
    ...
    .content;
    crate::lsp::lsp_open_document_with_app(
        Some(app),
        workspace_path.to_string(),
        language_id.clone(),
        path.clone(),
