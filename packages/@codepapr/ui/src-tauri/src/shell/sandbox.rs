...
mod tests {
    ...
    /// 隔离复现：后端在宿主内被静默 SIGKILL。用与 app_start 完全相同的 profile
    /// 在宿主外跑 novel-studio 后端（沙箱实例 + 非沙箱对照），观察是否同样死亡。
    /// 耗时约 6 分钟，默认 #[ignore]，手动运行：
    /// cargo test repro_backend_silent_death -- --ignored --nocapture
    #[test]
    #[ignore]
    fn repro_backend_silent_death() {
        use std::io::Read;
        use std::os::unix::process::ExitStatusExt;
        use std::process::{Command, Stdio};
        use std::time::{Duration, Instant};

        let workspace = PathBuf::from("/Users/example/Projects/Novel");
        let app_dir = workspace.join(".CodePapr/apps/novel-studio");
        let script = app_dir.join("server.js");
        if !script.exists() {
            eprintln!("SKIP: novel-studio 不存在");
            return;
        }

        // 与 app_start 相同的两轴：network=true(local 无关), workspace_write=false(local=read), allow_bind=true
        let access = SandboxAccess { network: true, workspace_write: false, allow_bind: true };
        let profile = build_profile("node", &workspace, Some(access), Some(app_dir.as_path()))
            .expect("profile should build");
        eprintln!("=== profile 长度: {} 字节 ===", profile.len());

        let spawn_instance = |label: &str, sandboxed: bool, port: u16| {
            let mut cmd = if sandboxed {
                let mut c = Command::new("/usr/bin/sandbox-exec");
                c.arg("-p").arg(&profile).arg("node").arg(&script);
                c
            } else {
            ...
            cmd.current_dir(&workspace)
                ...
                .stdout(Stdio::piped())
                .stderr(Stdio::piped());
            let mut child = cmd.spawn().expect("spawn should work");
            let pid = child.id();
            if let Some(mut reader) = child.stdout.take() {
                let label = format!("{label}/out");
                std::thread::spawn(move || {
                    let mut buf = [0u8; 4096];
                    loop {
                        match reader.read(&mut buf) {
                            Ok(0) | Err(_) => break,
                            Ok(n) => eprint!("[{label}] {}", String::from_utf8_lossy(&buf[..n])),
                        }
                    }
                });
            }
            if let Some(mut reader) = child.stderr.take() {
                let label = format!("{label}/err");
                std::thread::spawn(move || {
                    let mut buf = [0u8; 4096];
                    loop {
                        match reader.read(&mut buf) {
            ...
            eprintln!("=== {label} 已 spawn: pid={pid} port={port} sandboxed={sandboxed} ===");
            child
        };

        let mut sandboxed_child = spawn_instance("SANDBOXED", true, 18794);
        let mut control_child = spawn_instance("CONTROL", false, 28794);

        let port_bound = |port: u16| {
            std::net::TcpStream::connect(("127.0.0.1", port)).is_ok()
        };
        let start = Instant::now();
        // 等待两个实例完成监听（最多 30s）
        while start.elapsed() < Duration::from_secs(30) {
            if port_bound(18794) && port_bound(28794) {
                break;
            }
            std::thread::sleep(Duration::from_millis(200));
        }
        eprintln!(
            "=== 监听状态 @{:?}: sandboxed(18794)={} control(28794)={} ===",
            start.elapsed(),
            port_bound(18794),
            port_bound(28794)
        );

        // 观察 6 分钟，每 5 秒报告存活状态；任一死亡立即记录死因
        let watch_deadline = Duration::from_secs(360);
        let mut sandboxed_death: Option<String> = None;
        let mut control_death: Option<String> = None;
        let mut last_report = Instant::now();
        while start.elapsed() < watch_deadline {
            std::thread::sleep(Duration::from_secs(5));
            if sandboxed_death.is_none() {
                if let Some(status) = sandboxed_child.try_wait().ok().flatten() {
                    sandboxed_death = Some(format!(
                        "@{:?} code={:?} signal={:?}",
                        start.elapsed(),
                        status.code(),
                        status.signal()
                    ));
                    eprintln!("!!! SANDBOXED 死亡: {}", sandboxed_death.as_ref().unwrap());
                }
            }
            if control_death.is_none() {
                if let Some(status) = control_child.try_wait().ok().flatten() {
                    control_death = Some(format!(
                        "@{:?} code={:?} signal={:?}",
                        start.elapsed(),
                        status.code(),
                        status.signal()
                    ));
                    eprintln!("!!! CONTROL 死亡: {}", control_death.as_ref().unwrap());
                }
            }
            if last_report.elapsed() >= Duration::from_secs(30) {
                last_report = Instant::now();
                eprintln!(
                    "--- {:?}: sandboxed_alive={} control_alive={} ---",
                    start.elapsed(),
                    sandboxed_death.is_none(),
                    control_death.is_none()
                );
            }
            if sandboxed_death.is_some() && control_death.is_some() {
                break;
            }
        }

        let _ = sandboxed_child.kill();
        let _ = control_child.kill();

        eprintln!(
            "=== 结论: sandboxed_death={sandboxed_death:?} control_death={control_death:?} ==="
        );
    }

    /// 复现 2：在沙箱实例上叠加宿主的周边行为（独立进程组、15s 端口探测、
    /// 15s 健康请求），看哪一种会触发死亡。观察 8 分钟。
    /// cargo test repro_backend_with_host_mimicry -- --ignored --nocapture
    #[test]
    #[ignore]
    fn repro_backend_with_host_mimicry() {
        use std::io::Read;
        use std::os::unix::process::ExitStatusExt;
        use std::process::{Command, Stdio};
        use std::time::{Duration, Instant};
        ...
        cmd.arg("-p")
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::piped());
        // 与宿主一致：独立进程组（prepare_new_process_group）
        #[cfg(unix)]
        {
        ...
        if let Some(mut reader) = child.stdout.take() {
            std::thread::spawn(move || {
                ...
                loop {
                    match reader.read(&mut buf) {
                        Ok(0) | Err(_) => break,
                        Ok(n) => eprint!("[out] {}", String::from_utf8_lossy(&buf[..n])),
                    }
                }
            });
        }
        if let Some(mut reader) = child.stderr.take() {
            std::thread::spawn(move || {
                ...
                loop {
                    match reader.read(&mut buf) {
                        Ok(0) | Err(_) => break,
                        Ok(n) => eprint!("[err] {}", String::from_utf8_lossy(&buf[..n])),
                    }
                }
            });
        }
        eprintln!("=== MIMIC 已 spawn: pid={pid} port={port}（进程组独立）===");

        // 宿主行为 1：每 15s check_port_available（TcpListener::bind 127.0.0.1:port）
        std::thread::spawn(move || loop {
            std::thread::sleep(Duration::from_secs(15));
            let _ = std::net::TcpListener::bind(("127.0.0.1", port));
        });
        // 宿主行为 2：前端每 15s GET /api/health（用 TcpStream 发原始 HTTP，避免引入 client）
        std::thread::spawn(move || loop {
            std::thread::sleep(Duration::from_secs(15));
        ...
        let start = Instant::now();
        let watch_deadline = Duration::from_secs(480);
        let mut last_report = Instant::now();
        loop {
            std::thread::sleep(Duration::from_secs(5));
            if let Some(status) = child.try_wait().ok().flatten() {
                eprintln!(
                    "!!! MIMIC 死亡 @{:?}: code={:?} signal={:?}",
                    start.elapsed(),
                    status.code(),
                    status.signal()
                );
            ...
            if start.elapsed() >= watch_deadline {
                eprintln!("=== MIMIC 存活满 8 分钟，宿主周边行为无法复现死亡 ===");
                let _ = child.kill();
                break;
            }
            if last_report.elapsed() >= Duration::from_secs(30) {
                last_report = Instant::now();
                eprintln!("--- {:?}: alive ---", start.elapsed());
            }
        }
    }
}
