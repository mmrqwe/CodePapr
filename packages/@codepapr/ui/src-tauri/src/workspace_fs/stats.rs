...
impl ThreadScan {
    fn maybe_emit_progress(&self) {
        ...
        if seen == 1 || seen % PROGRESS_EVERY_FILES == 0 {
            let _ = app.emit(
                PROGRESS_EVENT,
                ProjectStatsProgress {
                    workspace_path: self.progress_key.clone(),
                    files: seen,
                },
            );
        }
...
pub(crate) async fn compute_project_stats(
    ...
    let result = run_blocking_workspace_task(move || {
        scan_project_stats(&workspace_path, cancel, Some(app))
    })
    .await;
...
#[tauri::command]
pub(crate) fn cancel_project_stats(workspace_path: String) {
