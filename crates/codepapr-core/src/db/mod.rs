...
pub fn sync_user_zone_to_ledger(workspace_path: String) -> Result<(), String> {
...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub struct IngestLegacyMemoryResult {
    pub ingested: u32,
...
pub fn ingest_legacy_memory_md(workspace_path: String) -> Result<IngestLegacyMemoryResult, String> {
    let (workspace, ..) = project_db_path(&workspace_path)?;
    let memory_path = workspace.join(".CodePapr").join("memory.md");
...
pub fn load_latest_memory_recall(
    workspace_path: String,
    session_id: String,
