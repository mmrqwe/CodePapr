...
fn sidecar_script_candidates(resource_dir: Option<&Path>) -> Vec<PathBuf> {
    ...
    candidates.push(manifest.join("../../packages/@codepapr/ui/dist-sidecar/agent-runtime.mjs"));
    candidates.push(manifest.join("../packages/@codepapr/ui/dist-sidecar/agent-runtime.mjs"));
    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            ...
            candidates.push(dir.join("dist-sidecar").join("agent-runtime.mjs"));
        }
    }
    if let Ok(cwd) = std::env::current_dir() {
        candidates.push(cwd.join("packages/@codepapr/ui/dist-sidecar/agent-runtime.mjs"));
        candidates.push(cwd.join("dist-sidecar/agent-runtime.mjs"));
    }
    candidates
}

fn resolve_sidecar_script(resource_dir: Option<&Path>) -> Result<PathBuf, String> {
    sidecar_script_candidates(resource_dir)
        .into_iter()
...
fn emit_frames(sink: SharedEventSink, runtime_id: String, stdout: std::process::ChildStdout) {
    ...
    for line in reader.lines().map_while(Result::ok) {
        ...
        dispatch_sidecar_frame(&sink, &runtime_id, message);
    }
...
fn dispatch_sidecar_frame(sink: &SharedEventSink, runtime_id: &str, message: serde_json::Value) {
    ...
    if ty == "tool-request" && agent_runtime_tools::should_host_in_rust(&message) {
        ...
        std::thread::spawn(move || {
            agent_runtime_tools::handle_hosted_tool_request(sink, runtime_id, message);
        });
        return;
    }
    ...
    sink.emit(
        FRAME_EVENT,
        serde_json::to_value(FrameEvent {
...
pub fn agent_runtime_start(
    ...
    let script = resolve_sidecar_script(resource_dir.as_deref())?;
    ...
    let sink_frames = Arc::clone(&sink);
    let id_frames = runtime_id.clone();
    thread::spawn(move || emit_frames(sink_frames, id_frames, stdout));
    Ok(runtime_id)
}
