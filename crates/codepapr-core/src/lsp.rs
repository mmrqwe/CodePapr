...
fn emit_managed_lsp_status(
    ...
    if let Some(sink) = sink {
        sink.emit(
            ...
            serde_json::to_value(ManagedLspStatusEvent {
...
fn start_server(
    ...
    let reporter = sink.map(|s| {
        move |progress: ManagedLspProgress| {
            emit_managed_lsp_status(Some(s), workspace_path, language_id, progress)
        }
    });
...
fn ensure_running_server_handle(
    ...
    let started = (|| {
        let workspace = canonical_workspace(workspace_path)?;
        start_server(sink, workspace_path, language_id).map_err(|err| {
...
pub fn lsp_open_document_with_sink(
    sink: Option<&dyn EventSink>,
    workspace_path: String,
    language_id: String,
    ...
    match ensure_running_server_handle(sink, &workspace_path, &language_id).and_then(|server| {
...
pub(crate) fn lsp_batch_symbols_with_limits(
    ...
    for (_, group) in by_language {
        ...
        for file in group.iter().copied() {
            ...
            let _ = lsp_open_document_with_sink(
                None,
...
mod tests {
    use super::{
        ...
        lsp_close_document_impl, lsp_open_document_with_sink, lsp_query_availability_impl,
