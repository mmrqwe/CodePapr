...
角色系统允许用户创建、导入和激活 AI 角色。激活后角色人设进入 Session Bootstrap（不进入 ImmutablePrefix），切换角色不破坏系统前缀缓存。

**角色数据结构（CharacterProfile）：** 名称、头像、描述、个性、场景、开场白、示例对话、系统提示词、标签、创作者、版本号、使用方式（编码人设 / 角色扮演）。
