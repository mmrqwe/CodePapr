...
> **2026-08 redesign + ADR-010 zero-review writes**: Memory Ledger (SQLite) +
> dual-zone `memory.md` projection + turn-scoped Recall. Memories **save
> automatically**; the user only browses / forgets / hand-edits the User Zone.
> Decisions: `docs/adr/` (ADR-001 ~ ADR-010); layering in §16.
...
| --- | --- | --- | --- |
| Hand-written notes | `memory.md` User Zone | Session Bootstrap (stable prefix) | After the user edits the file: **next session** or **compaction epoch** |
| Preferences / constraints / project facts | SQLite `memory_entries` → managed zone | Same, Bootstrap | Written to disk immediately; **current-session prefix stays frozen** until next session or compaction |
| Procedures (`procedure`) | ledger, not managed zone | Turn-scoped Recall / `memory_search` | Recallable from the next user turn; never in the prefix |
| Web / MCP citations (`citation`) | ledger, not managed zone | `memory_search` only (auto-Recall **skips** them) | Searchable once saved; never treated as instructions |
...
- **Managed Zone**: projection of Bootstrap-class `memory_entries` only
  (preference / constraint / fact / convention / verification / decision / api /
  general), regenerated by `project_memory_file` (Rust); capped at 24 entries /
  1500 tokens. `citation` / `procedure` / `user-note` stay out.
...
- **Direct write/patch of `memory.md`**: intercepted onto the same policy.

The panel is a catalog (every-session vs on-demand badges, forget). No admit /
reject.
...
- `memory.md` is excluded from `bootstrapSignature`: newly saved memories **do
...
post-reply), fire-and-forget. Consolidation rewrites the projection file; it
does not replace the ledger write policy.
...
- `packages/@codepapr/ui/src/utils/memoryLedger.ts`: extraction / projection rendering
- `packages/@codepapr/ui/src/utils/memoryPersist.ts`: auto persist (no user queue)
- `packages/@codepapr/ui/src/store/internals/memoryLedgerStore.ts`: end-of-turn orchestration
...
- `packages/@codepapr/ui/src-tauri/src/db/mod.rs`: memory_entries / memory_candidates /
  memory_recalls tables; `project_memory_file` (dual-zone projection)
...
L4. Project Memory (SQLite memory_entries + dual-zone memory.md projection)
    ...
    citation/procedure never enter Bootstrap.
...
- `packages/@codepapr/ui/src/utils/memoryLedger.ts`: Memory extraction / dual-zone projection rendering (pure)
- `packages/@codepapr/ui/src/utils/memoryPersist.ts`: Auto persist (no user queue)
