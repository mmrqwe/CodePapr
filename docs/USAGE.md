...
| 卡点 | 时机 | 触发条件 |
| --- | --- | --- |
| 交付 | 回合结束 | 多信号门：显式记忆意图（记住 / 以后都 / please remember…）、持久禁令（不要再用 / never … again）、Agent 在最终答复里的「记忆候选：」提议，或本回合有验证成功的测试/构建命令。Ask 模式不触发；普通模态词（必须 / always…）不触发。另有「预算压力兜底」：文件达 90% 预算时无需信号也整理（见下） |
| 压缩前 | 任意压缩入口（回合末 epoch、`/compact`、Goal、mid-loop）提交前 | 无条件运行，20s 超时（超时即中止在飞会话，本 epoch 沿用压缩前记忆） |

管家只看「本回合用户原话 + assistant 最终文本」（压缩卡点看将被折叠的骨架），**绝不含原始工具输出**。输出完整文件内容或 `NO_CHANGE`；改完立即落盘。

**预算压力整理**（ADR-017）：文件用量达硬顶的 90%（约 3600 tokens 或 108 行）时，交付卡点即使没有任何信号也会跑一次整理——`consolidate` 模式只做压缩：合并重复/同族条目、精简冗长表述、删除过时或可即时探测的条目，把文件压回目标线（≤3200 tokens / ≤100 行），稳定事实一条不丢；压缩前卡点在同样条件下也切整理模式。管家提示词始终携带当前用量与输出上限，避免满额重写被硬顶整体拒绝。整理未落盘且文件未变时不再重复触发（节流）；整理成功写入后若仍超线，下一回合继续迭代压缩。

**机械兜底**：密钥自动脱敏；注入指令 / 危险命令 / 策略绕过的内容拒写；超过 120 行或约 4000 tokens 拒写；「零新增且删除过半」视为洗记忆，拒写（列表 / 表格 / 段落统一计数，标题与表格分隔行除外）；保存时文件被并发修改则拒绝，面板重载后可重试。**shell 路径**：Agent 的 `write / patch / diff` 直写被拒；`bash / exec` 在 macOS 由内核沙箱整树拦截，Windows/Linux 由命令前后比对兜底回滚（命令新建记忆文件只告警不删——无法区分瞬时读取失败，避免误删用户数据）——记忆文件只由管家与面板维护。
