...
**机械兜底**：密钥自动脱敏；注入指令 / 危险命令 / 策略绕过的内容拒写；超过 120 行或约 4000 tokens 拒写；「零新增且删除过半」视为洗记忆，拒写（列表 / 表格 / 段落统一计数，标题与表格分隔行除外）；保存时文件被并发修改则拒绝，面板重载后可重试。**shell 路径**：Agent 的 `write / patch / diff` 直写被拒；`bash / exec` 在 macOS 由内核沙箱整树拦截，Windows/Linux 由命令前后比对兜底回滚——记忆文件只由管家与面板维护。
