...
2. **选入口**：使用桌面工作台。
3. **选模式**：Ask（解释分析）、Plan（拆解方案）、Agent（实际执行）。
...
- **重置到此点**：将该消息之后的所有对话和代码变更全部回退。利用 Git auto-commit checkpoint + `git reset --hard` 原子性恢复文件。非 Git 仓库自动降级到逐文件撤销。
- **复制**：一键复制消息原文到剪贴板。
...
**写入**：Agent 在以下三种情况下自动追加（用通用 `write` 工具）：
...
按优先级读取 `.CodePapr/AGENTS.md` → `.CodePapr/rules.md`，内容注入系统提示词。适合写：项目约定、目录结构、禁止改动的范围、验证标准。
