...
| --- | --- | --- |
| Delivery | End of turn | Multi-signal: explicit memory intent (“remember / from now on / please remember…”), durable prohibitions (“never use … again”), the Agent's `Memory candidate:` proposal in its final answer, or a verified test/build command. Never in Ask mode; plain modal words (“must / always…”) do not trigger. Plus a budget fallback: at 90% budget the file is condensed even without any signal (see below) |
...
**Budget-pressure consolidation** (ADR-017): once the file reaches 90% of the hard cap (~3600 tokens or 108 lines), the delivery checkpoint runs the curator even with no signal — in `consolidate` mode it only compacts: merging duplicate or same-family entries, tightening verbose wording, dropping outdated or instantly-detectable entries, and bringing the file back to the target line (≤3200 tokens / ≤100 lines) without losing stable facts. The pre-compaction checkpoint switches to the same mode under the same condition. The curator prompt always carries the current usage and the output ceiling so a full-file rewrite is not rejected wholesale by the hard cap. If a consolidation attempt does not land and the file is unchanged, it is not retried (throttle); after a successful write that is still over the line, the next turn compacts again iteratively.

**Mechanical guardrails**: secrets are redacted; injection instructions / dangerous commands / policy bypasses are rejected; files over 120 lines or ~4000 tokens are rejected; “no additions while dropping more than half the items” is treated as memory-washing and rejected (lists, tables, and paragraphs all count; headings and table separators do not); a concurrent edit causes a conflict, reload and retry from the panel. **Shell paths**: the Agent's `write / patch / diff` are rejected; `bash / exec` is blocked kernel-level on macOS and rolled back by a before/after check on Windows/Linux (a command that creates the memory file only raises a warning — a transient read failure cannot be told apart, so nothing is deleted) — the file is only maintained by the curator and the panel.
